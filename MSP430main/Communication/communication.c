...
uint8_t RX_ADDRESS[]= {0xE1, 0xE2, 0xE3, 0xE4, 0xE5};    //本机默认接收地址
...
void PollingNRF()
{
    ...
    state = ReadReg(RF_STATUS);
    WriteReg(SPI_WRITE_REG + RF_STATUS, state);
    
    if(state & MAX_TX)
    {
        WriteReg(RF_STATUS, MAX_TX);
